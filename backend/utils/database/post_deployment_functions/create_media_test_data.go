...
func createTestMedia(database *gorm.DB) {
	media := db.Media{
		...
		PosterImage: "https://example.com/test_poster.jpg",
		MediaTypeId: 1, // Assuming there's a MediaType record with ID 1
		CreatedAt:   timePtr(time.Now()),
	}
	database.Create(&media)
	fmt.Println("Media with ID 1 created.")
}
...
func createCurrentlyWatchingEntry(database *gorm.DB) {
	...
	fmt.Println("CurrentlyWatching entry created for User 1 and Media 1.")
}
